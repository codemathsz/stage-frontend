 ...
 */
  const calculatePhaseStartDate = (phaseIndex: number, phases: any[]): Date => {
    const startDate = new Date(projectVersion!.startDate);
  ...
  const calculatePhaseDates = (phases: ProjectPhase[], startDate: Date) => {
    ...
    currentDate.setDate(currentDate.getDate() + 1); // Adiciona 1 dia à data inicial

    return phases.map((phase, index) => {
      if (phase.isIndependent && phase.independentDate) {
        currentDate = new Date(phase.independentDate); // Usa a data independente se existir
      } else if (index > 0) {
        const previousPhase = phases[index - 1];
        // Adiciona as semanas da fase anterior à data atual
        currentDate.setDate(
          currentDate.getDate() + (previousPhase.weeks ?? 0) * 7
        );
      }

      // Retorna a fase atual com a data de início atualizada
      return {
        ...phase,
        startDate: new Date(currentDate), // Garante que cada fase tenha uma cópia própria da data
      };
    });
  };
  ...
  const drawChart = (
    ...
    const projectPhases = calculatePhaseDates(
      projectVersion?.phases.filter(
        (phase) => phase.phaseType === PhaseType.PROJECT
      ),
      projectVersion.startDate
    );
    const constructionPhases = calculatePhaseDates(
      projectVersion?.phases.filter(
        (phase) => phase.phaseType === PhaseType.CONSTRUCTION
      ),
    ...
    while (currentWeek < totalWeeks) {
      ...
      currentWeek = getWeeksBetweenDates(
        new Date(projectVersion.startDate).toISOString(),
    ...
    projectPhases.forEach((phase, index) => {
      const phaseStartWeek = getWeeksBetweenDates(
        new Date(projectVersion.startDate).toISOString(),
      ...
      phase.milestones.forEach((milestone, milestoneIndex) => {
        ...
        const milestoneWeeks =
          (milestoneDate.getTime() - phase.startDate.getTime()) /
          (7 * 24 * 60 * 60 * 1000);
        const milestoneX = x + milestoneWeeks * weekWidth;
        const milestoneY =
          currentY +
          phaseHeight +
          milestoneIndex * (milestoneRadius * 2 + milestonePadding);
        ...
        ctx.fillText(
          milestone.name,
          milestoneX + milestoneRadius + milestonePadding,
          milestoneY + 3
        );
      });
      ...
      currentY +=
        phaseHeight +
        phase.milestones.length * (milestoneRadius * 2 + milestonePadding) +
        10;
    });
    ...
    constructionPhases.forEach((phase, index) => {
      const phaseStartWeek = getWeeksBetweenDates(
        new Date(projectVersion.constructionStartDate).toISOString(),
        phase.startDate.toISOString()
      );
      const x = 200 + phaseStartWeek * weekWidth;
      const barWidth = phase.weeks * weekWidth;
  ...
  const exportToImage = async () => {
    ...
    const projectPhases = projectVersion?.phases.filter(
      (phase) => phase.phaseType === PhaseType.PROJECT
    );
    const constructionPhases = projectVersion?.phases.filter(
      (phase) => phase.phaseType === PhaseType.CONSTRUCTION
    );
    const allPhases = [...projectPhases, ...constructionPhases];
    const totalWeeks = allPhases.reduce((sum, phase) => sum + phase.weeks, 0);
    ...
    const exportWidth = Math.max(
      canvas.width,
      (totalWeeks * 20 + 200) * exportZoom
    );
    const exportHeight = Math.max(
      canvas.height,
      (totalHeight + 200) * exportZoom
    );
    ...
    tempCtx.fillText(
      `${projectVersion?.title} • ${projectVersion.title}`,
      20,
      60
    );
    ...
    tempCtx.fillText(
      `Início do Projeto: ${formatDate(
        projectVersion.startDate.toISOString().split("T")[0]
      )}`,
      20,
      120
    );
    ...
    tempCtx.fillText(
      `Início da Obra: ${formatDate(
        projectVersion.constructionStartDate.toISOString().split("T")[0]
      )}`,
      20,
      180
    );
    ...
    tempCtx.fillText(
      `Fim da Obra: ${formatDate(
        constructionEndDate.toISOString().split("T")[0]
      )}`,
      20,
      210
    );
    ...
    try {
      ...
      link.download = `gantt_${projectVersion?.title}_${
    ...
    <div className="relative">
      ...
      <div className="mt-4">
        <Slider
          ...
          onValueChange={([value]) => setHorizontalScroll(value)}
          max={1}
          step={0.01}
          className="w-full"
        />
      ...
      <div className="absolute top-20 right-2 h-[560px]">
        <Slider
          ...
          onValueChange={([value]) => setVerticalScroll(value)}
          max={1}
          step={0.01}
